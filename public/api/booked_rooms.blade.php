...
if (isset($_GET['booked_rooms'], $_GET['bus_id']) && !empty($_GET['bus_id'])) {
    $bus_id = Database::clean($_GET['bus_id']);
    $booked = $chambre->getBookedRoomsByBusId($bus_id); // Assuming you have a method to get rooms by bus ID
    echo json_encode(array_map(static function ($arr) {
        return $arr['numero_de_chambre'];
    }, $booked));
}

if (isset($_GET['show_booked_rooms'], $_GET['reservation_id']) && !empty($_GET['reservation_id'])) {
    $reservation_id = Database::clean($_GET['reservation_id']);
    $booked = $chambre->getBookedRooms($reservation_id);
            ...
            if ($customer->customerExists()){
                if($customer->book($seat)){
                    if (!$customer->sendMail($customer->getEmail(),$customer->generateMessage($seat))){
                        ...
                        echo json_encode(['success'=>'Booked successfully']);
                    }
                }
            }else{
